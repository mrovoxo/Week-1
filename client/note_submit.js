...
e.preventDefault();
	...
	if (note == "cheese" || noteTitle == "cheese") {
		alert("You can't insert empty note. Try to write something funny instead! :)" + note + " : " + noteTitle);
	} else {
		alert("You cannnnn insert empty note. Try to write something funny instead! :)" + note + " : " + noteTitle);
		//Meteor.call('noteInsert', note);
		post._id = Notes.insert(post);
